...
function template_ext($template,$config=array())
{
    ...
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\.([a-z0-9_]+)\s+empty\=\"(.*)\"\}/i", "<?php if(!empty($1[\'$2\'][\'$3\'])){echo $1[\'$2\'][\'$3\'];}else{echo '$4';}?>", $template);

    //不存在默认值
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\s+isset\=\"(.*)\"\}/iU", "<?php if(isset($1['$2'])){echo $1['$2'];}else{echo '$3';}?>", $template);
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\.([a-z0-9_]+)\s+isset\=\"(.*)\"\}/i", "<?php if(isset($1[\'$2\'][\'$3\'])){echo $1[\'$2\'][\'$3\'];}else{echo '$4';}?>", $template);
