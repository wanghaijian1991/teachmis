...
/*		if ( $url && @strpos($url,$script_name,0) !== false ){
    ...
    private function _define() {
        ...
        $script_name=$arr[0]."index.php";
		$root = $this->appConfig['URL_HTTP_HOST'] . str_replace(basename($script_name), '', $script_name);
		//__ROOT__和__PUBLIC__常用于图片，css，js定位，__APP__和__URL__常用于网址定位
		
		define('__ROOT__', substr($root, 0, -1));//当前入口所在的目录，后面不带 "/"
        $root=explode("index.php",__ROOT__);
		define('__PUBLIC__', $root[1] . '/' . 'public');
