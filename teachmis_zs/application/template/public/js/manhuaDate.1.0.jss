...
$(function() {
	$.fn.manhuaDate = function(options) {
		...
		$mhInput.live(options.Event,function(e){											
			...
			var iof = $(this).offset();
			$(".calenders").css({ "left" : iof.left+options.Left,"top" : iof.top+options.Top });
			$(".calenders").show();	
		});		
		//当鼠标离开控件上面的时候延迟3秒关闭
		$(".calenders").live("mouseleave",function(){ 
		...
		//当鼠标移到控件上面的时候显示
		$(".calenders").live("mousemove",function(){     
			if(stc){
				clearTimeout(stc);//清除定时器
			}
			$(this).show();
